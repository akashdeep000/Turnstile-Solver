...
if [ "$RUN_API_SOLVER" = "true" ]; then
    ...
    xvfb-run -a python3 /root/Desktop/Turnstile-Solver/api_solver.py --browser_type chrome --host 0.0.0.0 --default_timeout "${DEFAULT_TIMEOUT:-60}" --debug "${DEBUG_SOLVER:-false}"
else
